...
def search_bayesian(request):
    return render(request, 'search_bayesian.html', {})
...
# using beyesian networks 

def predict_bayesian(request, ticker_value, number_of_days):
    try:
        ticker_value = ticker_value.upper()
        df = yf.download(tickers=ticker_value, period='1d', interval='1m')
        print("Downloaded ticker = {} successfully".format(ticker_value))
    ...
    df_ml = df_ml[['Adj Close']].dropna()
    df_ml['Return'] = df_ml['Adj Close'].pct_change().dropna()

    data = df_ml[['Return']].copy().dropna()
    data['Return'] = data['Return'].apply(lambda x: 'Increase' if x > 0 else 'Decrease')

    model = BayesianNetwork([('Return', 'Return')])  # Simplified example of network structure
    ...
    for i in range(0, number_of_days):
        prediction = inference.map_query(variables=['Return'])
        pred_dict["Date"].append(dt.datetime.today() + dt.timedelta(days=i))
        pred_dict["Prediction"].append(prediction['Return'])
    ...
    return render(request, "predict_bayesian.html", context={'plot_div': plot_div,
                                                   'ticker_value': ticker_value,
                                                   'number_of_days': number_of_days,
                                                   'plot_div_bayes': plot_div_pred,
                                                   'Symbol': Symbol,
                                                   'Name': Name,
                                                   'Last_Sale': Last_Sale,
                                                   'Net_Change': Net_Change,
